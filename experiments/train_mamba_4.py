...
class BiMambaLayer(nn.Module):
    def __init__(self, dim, d_state=16, d_conv=4, expand=2):
        super().__init__()
        # NOVELTY 1: Two Mamba blocks for Bi-directional Context
        ...
        # NOVELTY 2: Local 3D Depthwise Conv to capture stroke edges
    ...
    def forward(self, x):
        ...
        # 2. Backward Scan (Flip the sequence so Mamba sees the "future")
        x_rev = torch.flip(x_norm, dims=[1])
        out_bwd = self.mamba_bwd(x_rev)
        out_bwd = torch.flip(out_bwd, dims=[1]) # Flip it back to original order
        
        # Combine Forward and Backward
        global_feat_flat = out_fwd + out_bwd
        global_feat = global_feat_flat.transpose(1, 2).view(b, c, h, w, d)
        
        # Combine Local, Global, and Residual Connection
        return global_feat + local_feat + x

class MambaUNet(nn.Module):
    def __init__(self, in_channels=3, out_channels=2, features=(16, 32, 64, 128, 256)):
        super().__init__()
        ...
        self.mamba_block = BiMambaLayer(dim=features[4]) # Updated to Bi-Mamba
    ...
    def forward(self, x):
        ...
        b = self.mamba_block(self.bottleneck_conv(e4))
