...
# =========================
# 1. CLASSES (No Imports to avoid training loop triggers)
# =========================
...
# =========================
# 3. TEST DATA PREP (IMAGES + LABELS)
# =========================
# Saari images aur labels load kar rahe hain
test_images = sorted(glob.glob(os.path.join(DATA_DIR, "imagesTr", "*.nii.gz")))
test_labels = sorted(glob.glob(os.path.join(DATA_DIR, "labelsTr", "*.nii.gz")))

assert len(test_images) == len(test_labels), "Images aur Labels match nahi kar rahe!"
...
with torch.no_grad():
    for i, batch_data in enumerate(test_loader):
        ...
        # Format outputs for Dice Calculation
        val_labels = decollate_batch(labels)
        val_outputs = [final_prediction.squeeze(0)] # Strip batch dim
        
        # Calculate metric for this specific image
        dice_metric(y_pred=val_outputs, y=val_labels)
        print(f"✅ Evaluated Scan {i+1}/{len(test_files)}")

# Aggregate saari 250+ images ka average nikalega
final_dice = dice_metric.aggregate().item()

print("\n" + "="*50)
print(f"🏆 MASTER ENSEMBLE DICE SCORE (FULL DATASET): {final_dice:.4f}")
print("="*50 + "\n")