...
BATCH_SIZE = 2
MAX_EPOCHS = 200          # <-- Updated to 200 (Proven optimal limit)
VAL_INTERVAL = 2
NUM_WORKERS = 2
NUM_FOLDS = 5             # <-- 5-Fold Ensemble Setup
...
# =========================
# THE BEAST: Tri-Planar OmniMamba-UNet
# =========================
...
print("🌟 Initiating 5-Fold Cross Validation for ISLES 2022...", flush=True)

for fold, (train_idx, val_idx) in enumerate(kf.split(data)):
    print(f"\n==========================================")
    print(f"🚀 STARTING FOLD {fold} / {NUM_FOLDS-1}")
    print(f"==========================================")
    ...
    print(f"Fold {fold} | Train: {len(train_files)} | Validation: {len(val_files)}", flush=True)

    # 2. Setup DataLoaders
    train_ds = Dataset(data=train_files, transform=train_transforms)
    val_ds = Dataset(data=val_files, transform=val_transforms)
    
    train_loader = DataLoader(train_ds, batch_size=BATCH_SIZE, shuffle=True, num_workers=2) # num_workers reduced to 2
    val_loader = DataLoader(val_ds, batch_size=1, shuffle=False, num_workers=2)

    # 3. Initialize Fresh Model & Optimizer for this fold
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        if (epoch + 1) % 10 == 0 or epoch == 0: # Print less to keep logs clean
            print(f"Fold {fold} - Epoch [{epoch+1}/{MAX_EPOCHS}] Loss: {epoch_loss:.4f}", flush=True)
        ...
        if (epoch + 1) % VAL_INTERVAL == 0:
            ...
            with torch.no_grad():
                ...
                if metric > best_metric:
                    best_metric = metric
                    # Save specifically for this fold
                    torch.save(model.state_dict(), f"best_mamba_model_fold_{fold}.pth")
                    print(f"⭐ Fold {fold} - New best Dice: {metric:.4f}", flush=True)

    print(f"✅ Fold {fold} Completed in {(time.time()-fold_start_time)/60:.2f} minutes. Best Dice: {best_metric:.4f}")
    
    # 5. Clear memory before starting next fold
    del model, optimizer, train_loader, val_loader, train_ds, val_ds
    torch.cuda.empty_cache()

print(f"\n🎉 5-Fold Training Fully Completed in {(time.time()-global_start_time)/3600:.2f} hours!", flush=True)