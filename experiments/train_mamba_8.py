...
# =========================
# THE NOVELTY: Hybrid Focal-Tversky + Adaptive Wing Loss
# =========================
class FocalTverskyAdaptiveWingLoss(nn.Module):
    def __init__(self, alpha=0.3, beta=0.7, gamma=0.75, omega=14.0, theta=0.5, epsilon=2.0, alpha_w=2.1):
        super().__init__()
        ...
        # Adaptive-Wing Params
        self.omega = omega
        self.theta = theta
        self.epsilon = epsilon
        self.alpha_w = alpha_w
        
        # 🔧 FIX: Pre-calculate the constant log term purely in Python math
        # Isse PyTorch ka "float vs Tensor" error nahi aayega aur speed bhi badhegi
        self.awl_constant = math.log(1 + (self.theta / self.epsilon)**self.alpha_w)

    def forward(self, y_pred, y_true):
        # 1. Convert logits to probabilities
        probs = torch.softmax(y_pred, dim=1)
        probs_fg = probs[:, 1, ...] # Foreground
        
        # 2. Convert ground truth to one-hot
        y_true_onehot = one_hot(y_true, num_classes=2, dim=1)
        targets_fg = y_true_onehot[:, 1, ...]
        
        # Flatten
        p_flat = probs_fg.contiguous().view(-1)
        t_flat = targets_fg.contiguous().view(-1)
        
        # --- PART A: FOCAL-TVERSKY LOSS ---
        TP = (p_flat * t_flat).sum()
        FP = ((1 - t_flat) * p_flat).sum()
        FN = (t_flat * (1 - p_flat)).sum()
        
        Tversky = (TP + 1e-5) / (TP + self.alpha * FP + self.beta * FN + 1e-5)
        Focal_Tversky = (1 - Tversky) ** self.gamma
        
        # --- PART B: ADAPTIVE-WING LOSS ---
        diff = torch.abs(t_flat - p_flat)
        
        # 🔧 FIX Applied: Using self.awl_constant instead of torch.log for the scalar part
        loss_awl = torch.where(
            diff < self.theta,
            self.omega * torch.log(1 + (diff / self.epsilon)**self.alpha_w),
            diff - self.theta + self.omega * self.awl_constant 
        ).mean()
        
        # --- FUSION ---
        total_loss = Focal_Tversky + (0.05 * loss_awl) 
...
loss_function = FocalTverskyAdaptiveWingLoss(alpha=0.3, beta=0.7, gamma=0.75)
