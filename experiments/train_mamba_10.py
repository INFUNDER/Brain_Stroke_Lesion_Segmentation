...
# =========================
# 1. TRI-PLANAR OMNIMAMBA LAYER (The Global Visionary)
# =========================
class OmniMambaLayer(nn.Module):
    def __init__(self, dim, d_state=32, d_conv=4, expand=2): 
        ...
        # 3 Separate Mamba Engines for X, Y, Z spatial planes
...
# =========================
# 2. NOVELTY: SPATIAL GATED MIXTURE OF EXPERTS (MoE)
# =========================
class SpatialGatedMoE(nn.Module):
    def __init__(self, dim):
        super().__init__()
        
        # EXPERT 1: Local Detail Expert (Focuses on tiny stroke dots)
        ...
        # EXPERT 2: Global Context Expert (Focuses on massive strokes)
        self.expert_global = OmniMambaLayer(dim=dim)
        
        # THE ROUTER: Pixel-level decision maker
        self.router = nn.Sequential(
            nn.Conv3d(dim, dim // 2, kernel_size=3, padding=1),
            nn.InstanceNorm3d(dim // 2),
            nn.GELU(),
            nn.Conv3d(dim // 2, 2, kernel_size=1), # 2 weights (one for each expert)
            nn.Softmax(dim=1) 
        )

    def forward(self, x):
        out_local = self.expert_local(x)
        out_global = self.expert_global(x)
        
        gate_weights = self.router(x) # Shape: (B, 2, H, W, D)
        
        weight_local = gate_weights[:, 0:1, ...] 
        weight_global = gate_weights[:, 1:2, ...]
        
        # Combine experts based on router's spatial weights
        final_out = (out_local * weight_local) + (out_global * weight_global)
        
        return final_out + x # Residual connection

# =========================
# 3. THE MASTERPIECE: MoE-OmniMamba-UNet
# =========================
class MambaUNet(nn.Module):
    def __init__(self, in_channels=3, out_channels=2, features=(32, 64, 128, 256, 512)):
        ...
        # Smart Skip Connections (Filtered by OmniMamba)
        ...
        # NOVEL BOTTLENECK: Mixture of Experts
        ...
        self.moe_bottleneck = SpatialGatedMoE(dim=features[4]) # <-- The Game Changer
    ...
    def forward(self, x):
        ...
        # 2. Bottleneck (MoE Routing)
        b = self.bottleneck_conv(e4)
        b = self.moe_bottleneck(b)

        # 3. Filter Skip Connections
