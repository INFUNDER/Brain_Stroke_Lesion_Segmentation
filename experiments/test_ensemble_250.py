...
OUTPUT_DIR = "/home/ronit.28010/Brain_Stroke/ensemble_outputs" 
...
# =========================
# 3. TEST DATA PREP (LOAD ALL IMAGES)
# =========================
# Yahan se humne [-20:] hata diya hai. Ab yeh folder ki SAARI images uthayega.
...
# =========================
# 4. INFERENCE & SAVE LOOP (BUG FIXED)
# =========================
print(f"🚀 Generating 3D Predictions for {len(test_files)} images and saving to: {OUTPUT_DIR}")
...
with torch.no_grad():
    for i, batch_data in enumerate(test_loader):
        ...
        # THE FIX: Ensuring Affine is strictly 2D (4x4 matrix) to avoid save crash
        for val_pred, val_input in zip(final_prediction, inputs):
            affine_matrix = val_input.affine
            # Agar batching ki wajah se 3D ho gaya hai, toh use 2D mein laayein
            if affine_matrix.ndim == 3:
                affine_matrix = affine_matrix[0] 
                
            if isinstance(val_input, monai.data.MetaTensor):
                pred_meta_tensor = monai.data.MetaTensor(val_pred, affine=affine_matrix, meta=val_input.meta)
            else:
